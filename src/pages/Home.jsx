...
const Home = () => {
  ...
  return (
    <main className="pt-28 space-y-24 px-4 max-w-7xl mx-auto">
      {/* Section Héros - Agrandie */}
      <section className="flex flex-col md:flex-row items-center gap-12 animate-fade-in py-12">
        <div className="flex-1 text-center md:text-left space-y-6">
          <h1 className="text-5xl md:text-6xl font-bold text-teal-600 dark:text-teal-300 leading-tight">
            {t("home_title")}
          </h1>
          ...
          <button
            className="bg-teal-500 text-white px-8 py-3 rounded-lg hover:bg-teal-600 transition text-lg font-semibold"
        ...
        <div className="flex-1 flex justify-center">
          <img
            ...
            className="w-full max-w-lg mx-auto animate-slide-in-right"
          />
        </div>
      </section>

      {/* À propos - Section plus grande */}
      <section className="max-w-6xl mx-auto text-center space-y-6 animate-slide-in-left py-12">
        <h2 className="text-4xl font-bold text-teal-600 dark:text-teal-300">{t("Who_are_we")}</h2>
        <p className="text-xl text-gray-700 dark:text-gray-300 px-4">
          {t("Description")}
        </p>
      </section>

      {/* Services - Cartes agrandies */}
      <section id="services" className="space-y-12 py-12">
        <h2 className="text-4xl font-bold text-center text-teal-600 dark:text-teal-300">Nos Services</h2>
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {servicesData.map((service, index) => (
            <div 
              key={index} 
              className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition transform hover:-translate-y-2 flex flex-col items-center text-center"
            >
              <img 
                src={service.image} 
                alt={service.title} 
              ...
              />
              <h3 className="font-bold text-2xl text-teal-700 dark:text-teal-400 mb-3">{service.title}</h3>
      ...
      {/* Technologies - Section agrandie */}
      <section className="text-center space-y-8 py-12">
        <h2 className="text-4xl font-bold text-teal-600 dark:text-teal-300">{t("technologies")}</h2>
        <div className="flex justify-center flex-wrap gap-8 text-6xl text-teal-500 dark:text-teal-300">
            <FaReact title="React" className="hover:scale-110 transition" />
            <FaNodeJs title="Node.js" className="hover:scale-110 transition" />
            <FaCss3Alt title="Tailwind CSS" className="hover:scale-110 transition" />
            <FaDatabase title="Database" className="hover:scale-110 transition" />
            <FaPython title="Python" className="hover:scale-110 transition" />
            <FaTools title="Maintenance Service" className="hover:scale-110 transition" />
            <FaCloud title="Cloud Services" className="hover:scale-110 transition" />
        </div>
      </section>

      {/* Produits Tech - Section agrandie */}
      <section id="products" className="space-y-8 py-12">
        <h2 className="text-4xl font-bold text-center text-teal-600 dark:text-teal-300">{t("products")}</h2>
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div 
              key={i} 
              className="bg-gray-100 dark:bg-gray-800 p-6 rounded-xl text-center hover:shadow-xl transition flex flex-col items-center"
            >
              <img src={p.image} alt={p.name} className="h-40 w-full object-contain mb-4" />
              <h4 className="font-semibold text-xl text-teal-600 dark:text-teal-300">{p.name}</h4>
            </div>
          ))}
        </div>
      </section>

      {/* Témoignages - Section agrandie */}
      <section className="text-center space-y-8 py-12">
        <h2 className="text-4xl font-bold text-teal-600 dark:text-teal-300">Ce que disent nos clients</h2>
        <div className="grid gap-8 md:grid-cols-3">
          {[
            ...
            <div key={i} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-left space-y-4">
              <FaQuoteLeft className="text-teal-400 text-3xl" />
              <p className="text-lg text-gray-700 dark:text-gray-300">{msg}</p>
            </div>
          ))}
        </div>
      </section>

      {/* Bouton Nous contacter - Agrandi */}
      <div className="text-center py-12">
        <button
          onClick={() => setContactOpen(true)}
          className="bg-teal-600 text-white py-4 px-10 rounded-xl hover:bg-teal-700 transition text-xl font-semibold shadow-lg"
        >
          {t("contact_us")}
        </button>
      </div>
